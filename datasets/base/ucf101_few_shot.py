...
class UCF101_few_shot(BaseVideoDataset):
    def __init__(self, cfg, split):
        super(UCF101_few_shot, self).__init__(cfg, split)
    ...
    def _construct_dataset(self, cfg):
        ...
        for retry in range(5):
            try:
                ...
                logger.info(
                    "Dataset {} split {} loaded. Length {}.".format(self.dataset_name, self.split, len(self._samples)))
                break
            except:
                if retry < 4:
                    continue
    ...
    def __getitem__(self, index):
        ...
        if self.cfg.TRAIN.META_BATCH:
            ...
            for retry in range(retries):
                try:
                    ...
                    for bl, bc in enumerate(batch_classes):
                        ...
                        # for retry in range(retries):
                        #     try:
                        idxs = random.sample([i for i in range(n_total)],
                                             self.cfg.TRAIN.SHOT + n_queries)  # 获取指定类所需视频索引

                        for idx in idxs[0:self.cfg.TRAIN.SHOT]:  # 因为是oneshot 只循环一次
                            if hasattr(self.cfg.AUGMENTATION,
                                       "SUPPORT_QUERY_DIFF_SUPPORT") and self.cfg.AUGMENTATION.SUPPORT_QUERY_DIFF_SUPPORT and self.split_dataset == "train":
                        ...
                        for idx in idxs[self.cfg.TRAIN.SHOT:]:  # query要五个样本 循环五次
                            if hasattr(self.cfg.AUGMENTATION,
                                       "SUPPORT_QUERY_DIFF") and self.cfg.AUGMENTATION.SUPPORT_QUERY_DIFF and self.split_dataset == "train":
                                vid, vid_id = self.get_seq_query(bc, idx)
                            else:
                                vid, vid_id = self.get_seq(bc, idx)
            ...
            support_set = torch.cat(support_set)  # [40, 3, 224, 224] 5*8=40

            target_set = torch.cat(target_set)  # [40, 3, 224, 224]
            ...
            return {"support_set": support_set, "support_labels": support_labels, "target_set": target_set,
                    ...
                    "batch_class_list": batch_classes, "real_support_labels": real_support_labels}
            ...
            for retry in range(retries):
                try:
                    data, file_to_remove, success = self.decode(
                        sample_info, index,
                        num_clips_per_video=self.num_clips_per_video if hasattr(self, 'num_clips_per_video') else 1
                    )
                    break
                except Exception as e:
                    success = False
            ...
            if "Slowfast" in self.cfg.VIDEO.BACKBONE.META_ARCH and self.split not in ['extract_feat']:
                slow_idx = torch.linspace(0, data["video"].shape[1],
                                          data["video"].shape[1] // self.cfg.VIDEO.BACKBONE.SLOWFAST.ALPHA + 1).long()[
                           :-1]
