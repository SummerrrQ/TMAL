...
class Split_few_shot():
    ...
    def __init__(self, folder, split_dataset='train', dataset="Ssv2_few_shot"):
        ...
        elif dataset == 'SSV2Full_few_shot':
            for class_folder in folder:
                paths = class_folder.strip().split('/')[-1]

                class_id = int(
                    class_folder.strip().split('/')[0][len(split_dataset):])  # class_folders.index(class_folder)
                self.add_vid(paths, class_id)
        else:
            for class_folder in folder:
                paths = class_folder.strip().split('//')[-1]
...
class ArgsObject(object):
    def __init__(self):
        ...
        self.noise = 'normal'
        self.current_iter = 1  # 循环次数
        self.lightCNN = 'shufflev2_0_5'  # 轻量级backbone
        self.sigma_step = 1000
        self.dropout = 0.1
        self.ada = True  # 用ada优化器
...
class Ssv2_few_shot(BaseVideoDataset):
    ...
    def _construct_dataset(self, cfg):
                    ...
                    with open(local_file) as f:
                        ...
                        for line in lines:
                            for idx in range(self._num_clips):
                                ...
                                self._spatial_temporal_index.append(idx)
                self.split_few_shot = Split_few_shot(lines, self.split, dataset=self.dataset_name) # 读取每一行 组成列表，然后拆开
        ...
        if hasattr(self.cfg.TRAIN, "FEW_SHOT") and self.cfg.TRAIN.FEW_SHOT and self.split == "train":
            """ Sample number setting for training in few-shot settings: 
                During few shot training, the batch size could be larger than the size of the training samples.在少镜头训练期间，批量大小可能大于训练样本的大小。
                Therefore, the number of samples in the same sample is multiplied by 10 times, and the training schedule is reduced by 10 times. 因此，同一样本中的样本数量乘以10倍，训练计划减少了10倍
            """
    ...
    def getsub(self,x):
        fame = x.size(0)
        fame_list = []
        # fame_list.append(x[0].permute(1, 2, 0))
        for i in range(fame-1):
            gray_before = cv2.cvtColor(x[i].permute(1, 2, 0).numpy(), cv2.COLOR_BGR2GRAY)
            gray_after = cv2.cvtColor(x[i+1].permute(1, 2, 0).numpy(), cv2.COLOR_BGR2GRAY)
            flow = cv2.calcOpticalFlowFarneback(gray_before, gray_after, None, 0.5, 3, 15, 3, 5, 1.1, 0)
            # gray_frame = torch.mean(x[i + 1],dim=0)
            # gray_frame_1 = torch.mean(x[i],dim=0)
            # flow = cv2.calcOpticalFlowFarneback(gray_frame, gray_frame_1, None, 0.5, 3, 15, 3, 5, 1.1, 0)S
            # sub = cv2.absdiff((x[i + 1].numpy()), (x[i].numpy()))
            # sub_gray = cv2.cvtColor(np.transpose(sub, (1, 2, 0)), cv2.COLOR_BGR2GRAY)
            # sub_gray = cv2.normalize(sub_gray, None, 0, 255, cv2.NORM_MINMAX)
            # thresh = cv2.adaptiveThreshold(sub_gray.astype(np.uint8), 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C,cv2.THRESH_BINARY, 11, 0)
            # thresh_flat =cv2.morphologyEx(thresh, cv2.MORPH_OPEN, np.ones((3, 3), np.uint8))
            # thresh_flat =  cv2.dilate(thresh_flat,np.ones((3,3),np.uint8),iterations =2)
            # re = cv2.bitwise_and((x[i+1].permute(1, 2, 0)).numpy(), (x[i+1].permute(1, 2, 0)).numpy(), mask=thresh_flat)
            # plt.imshow(re)
            # plt.savefig("re_{}.jpg".format(i))

            fame_list.append(torch.from_numpy(flow))
        fame_list = torch.stack(fame_list).permute(0,3,1,2)
        return fame_list
    def __getitem__(self, index):
        ...
        if self.cfg.TRAIN.META_BATCH:
            ...
            classes = c.get_unique_classes() # 一共有64类
            batch_classes = random.sample(classes, self.cfg.TRAIN.WAY) # 随机抽取5 way
            # batch_classes = [3,5,7,14,23]# [6, 7, 0, 3, 20]
            if self.split != "train" and hasattr(self.cfg.TRAIN, "WAT_TEST"):
                batch_classes = random.sample(classes, self.cfg.TRAIN.WAT_TEST)

            if self.split == "train":
                n_queries = self.cfg.TRAIN.QUERY_PER_CLASS # 5
            else:
            ...
            for retry in range(retries):
                try:
                    ...
                    real_support_labels = []
                    real_target_labels = []
                    support_subframe_set = []
                    target_subframe_set = []
                    for bl, bc in enumerate(batch_classes):
                        n_total = c.get_num_videos_for_class(bc) # 看该类中有多少样本
                        # retries = 5
                        # for retry in range(retries):
                        #     try:
                        idxs = random.sample([i for i in range(n_total)], self.cfg.TRAIN.SHOT + n_queries) # 获取指定类所需视频索引

                        for idx in idxs[0:self.cfg.TRAIN.SHOT]:# 因为是oneshot 只循环一次
                            if hasattr(self.cfg.AUGMENTATION, "SUPPORT_QUERY_DIFF_SUPPORT") and self.cfg.AUGMENTATION.SUPPORT_QUERY_DIFF_SUPPORT and self.split_dataset=="train":
                                vid, vid_id = self.get_seq_query(bc, idx) #bc是类，idx是随机索引
                            else: # here
                                vid, vid_id, flow = self.get_seq(bc, idx)
                            # sub_frame = self.getsub(vid)
                            support_subframe_set.append(flow)
                            support_set.append(vid)
                            support_labels.append(bl)
                            real_support_labels.append(bc)
                        
                        # try:
                        for idx in idxs[self.cfg.TRAIN.SHOT:]: # query要五个样本 循环五次
                            if hasattr(self.cfg.AUGMENTATION, "SUPPORT_QUERY_DIFF") and self.cfg.AUGMENTATION.SUPPORT_QUERY_DIFF and self.split_dataset=="train":
                                ...
                                vid, vid_id, flow = self.get_seq(bc, idx)
                            # sub_frame = self.getsub(vid)
                            target_subframe_set.append(flow)
            ...
            s = list(zip(support_set, support_labels, real_support_labels, support_subframe_set))
            random.shuffle(s)
            support_set, support_labels, real_support_labels,support_subframe_set= zip(*s)

            t = list(zip(target_set, target_labels, real_target_labels, target_subframe_set))
            random.shuffle(t)
            target_set, target_labels, real_target_labels,target_subframe_set= zip(*t)

            support_subframe_set = torch.stack(support_subframe_set)
            target_subframe_set = torch.stack(target_subframe_set)
            support_set = torch.stack(support_set)  # [40, 3, 224, 224] 5*8=40
            target_set = torch.stack(target_set)  # [40, 3, 224, 224]
            support_labels = torch.FloatTensor(support_labels) #【3，2，0，1，4】
            target_labels = torch.FloatTensor(target_labels) # 【3，1，2，0，4】
            real_target_labels = torch.FloatTensor(real_target_labels) # 5 # shape: [59，32，44，55，38]
            real_support_labels = torch.FloatTensor(real_support_labels) # 【59，44，55，32，38】
            ...
            return {"support_set":support_set, "support_labels":support_labels, "target_set":target_set, "target_labels":target_labels, "real_target_labels":real_target_labels, "batch_class_list": batch_classes,
                    ...
                    "support_subframe_set":support_subframe_set,"target_subframe_set":target_subframe_set}
    ...
    def get_seq(self, label, idx=-1):
        ...
        if self.cfg.TRAIN.META_BATCH:
            ...
            if self.dataset_name == 'Ssv2_few_shot':
                ...
                # print(video_path)
            elif self.dataset_name == 'SSV2Full_few_shot':
                video_path = os.path.join(self.data_root_dir, paths + ".mp4")
                # print(video_path)
            else:
            ...
            if self.dataset_name == 'Ssv2_few_shot':
                flow_path = video_path.replace("xiaqing/SSV2","data1/xiaqing/SSV2_flow2").replace(".mp4",".pt")
            elif self.dataset_name =="HMDB_few_shot":
                flow_path = video_path.replace("xiaqing/HMDB51", "data1/xiaqing/HMDB51_flow2").replace(".avi", ".pt")
            elif self.dataset_name =="UCF101_few_shot":
                flow_path = video_path.replace("xiaqing/UCF-101", "data1/xiaqing/UCF101_flow2").replace(".avi", ".pt")
            elif self.dataset_name =="Kinetics_few_shot":
                flow_path = video_path.replace("xiaqing/K100", "data1/xiaqing/K100_flow2").replace(".mp4", ".pt")
            elif self.dataset_name =='SSV2Full_few_shot':
                flow_path = video_path.replace("SSV2-FULL", "SSV2-Full_flow2").replace(".mp4", ".h5")
            # sample_info = self._get_sample_info(index)
            index = vid_id
            ...
            if not success:
                logger.info("Error at decoding. Vid index: {}, Vid path: {}".format(
                    index, sample_info["path"]))
                return self.__getitem__(index - 1) if index != 0 else self.__getitem__(index + 1)
            if data["video"].numel() == 0: # 返回数组中元素的个数
                logger.info("data[video].numel()=0. Vid index: {}, Vid path: {}".format(
                    index, sample_info["path"]))
                return self.__getitem__(index - 1) if index != 0 else self.__getitem__(index + 1)
            # if self.split in ["test"] and self.cfg.TEST.ZERO_SHOT:
            ...
            if  "Slowfast" in self.cfg.VIDEO.BACKBONE.META_ARCH and self.split not in ['extract_feat']:
                ...
                data["video"] = [slow_frames, fast_frames]
            bu.clear_tmp_file(file_to_remove)
            if self.dataset_name == 'SSV2Full_few_shot':
                with h5py.File(flow_path, 'r') as hf:
                    # 从数据集中读取数据
                    loaded_frame_array = np.array(hf[paths])
                    # 将 NumPy 数组转换为 PyTorch 张量
                    loaded_tensor = torch.from_numpy(loaded_frame_array)
            else:
                loaded_tensor = torch.load(flow_path)
            return data["video"].permute(1,0,2,3), vid_id,loaded_tensor
