...
class Ucf101(BaseVideoDataset):
    ...
    def _get_sample_info(self, index):
        ...
        """
        class_,video_path = self._samples[index].strip().split('//')
        # my_cla = video_path.split('/')[3]
        # my_dict = {}
        # for index, item in enumerate(self.cfg.TRAIN.CLASS_NAME):
        #     item = item.replace(' ', '')
        #     my_dict[item] = index

        class_ = int(class_[5:])
        video_path = video_path.replace('videos/', '')
