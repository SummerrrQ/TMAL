...
class ScalarMeter(object):
    """
    A scalar meter uses a deque to track a series of scaler values with a given  标量表使用deque跟踪一系列给定的标量值
    window size. It supports calculating the median and average values of the   窗口大小。它支持计算
    window, and also supports calculating the global average.   窗口，还支持计算全局平均值。
    """
...
class ValMeter(object):
    ...
    def log_epoch_stats(self, cur_epoch):
        ...
        self.best_epoch = cur_epoch+1 if self.min_top1_err == top1_err else self.best_epoch
        stats["top1_err"] = top1_err
        stats["top5_err"] = top5_err
